<template>
  <a-row class="trace-panel">
    <a-col class="panel-header">
      <a-button type="primary" @click="shrinkAll" v-if="collapsed">
        <a-icon type="shrink" />Collapse All
      </a-button>
      <a-button v-else @click="expandAll">
        <a-icon type="arrows-alt" />Expand All
      </a-button>
      <a-button type="primary" @click="addTrace('trace ' + traces.length)">
        <a-icon type="plus" />Trace
      </a-button>
    </a-col>
        ...
        <a-collapse-panel :key="trace.name" :header="trace.name">
          <a-row>
            ...
            <a-col :span="18">
              ...
              >
                <a-select-option
                  :value="plotType"
                  v-for="plotType in plotTypeList"
                  :key="plotType"
                >{{ plotType }}</a-select-option>
              </a-select>
            </a-col>
          </a-row>
          <a-row>
            ...
            <a-col :span="18">
              <a-select
                style="width: 100%"
                @change="changeTrace(trace.name, 'x', $event)"
          ...
          <a-row>
            ...
            <a-col :span="18">
              <a-select
                style="width: 100%"
                @change="changeTrace(trace.name, 'y', $event)"
                placeholder="Choose data..."
              >
                <a-select-option :value="field" v-for="field in fields" :key="field">{{ field }}</a-select-option>
              </a-select>
