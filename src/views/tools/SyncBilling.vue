<template>
  <page-view title="Aliyun Billing Syncer" logo="https://gw.alipayobjects.com/zos/rmsportal/nxkuOJlFJuAUhzlMTCEe.png">
    <template slot="action">
      <a-button type="primary" @click="onSyncData" :disabled="isRunning">New Task</a-button>
    </template>
    <a-table
    ...
    <a-modal v-model="modalVisible" title="Sync Billing" :footer="null">
      <a-form :form="form" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }" layout="horizontal" @submit="handleOk">
        <p>After you click the <b>Sync</b> button, the system will automatically sync the billing data to Metabase (as of the cut-off time to the current time)</p>
  ...
  methods: {
    ...
    handleOk(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          const baseUrl = `${initTServiceHost()}/api/tool/sync-billing`
          this.$http
            .post(baseUrl, values)
            .then(response => {
              console.log('Sync Billing: ', response)
              this.$message.success('Submit Successfully.')
              this.getTasks(this.pagination.current, this.pagination.pageSize)
            })
            .catch(error => {
              this.$message.success('Failed, Please Retry Later.')
              console.log('Sync Billing Error: ', error)
            })
          this.onCancel()
        }
      })
    },
    checkRunning(records) {
      const startedRecords = filter(records, record => {
    ...
    getTasks(page, pageSize) {
      ...
      this.$http
        ...
        .then(response => {
          ...
          this.isRunning = this.checkRunning(response.data)
